                    ...
                    const std::vector<std::vector<std::size_t>>&cov)
{
    ...
    for(size_t from=0;from<graph.size();++from)
    {
        ...
        {
            if(to>from) continue;
            weight+=graph[from][*find_edge(from,to,graph)].second;
        }
    }
    return weight;
}
                       ...
                       const std::vector<std::size_t>&match)
{
    ...
    for(size_t from=0;from<graph.size();++from)
    {
        size_t to=match[from];
        if(to==from) continue;
        if(to>=graph.size()||match[to]!=from||!find_edge(to,from,graph)) return false;
    }
    return true;
}
                       ...
                       const std::vector<std::vector<std::size_t>>&cov)
{
    ...
    for(size_t from=0;from<graph.size();++from)
    {
        ...
        for(size_t to:cov[from])
        {
            if(to>=graph.size()||from==to||!find_edge(from,to,graph)||!find_edge(to,from,cov))return false;
        }
    }
    return true;
}
...
int bipartite_solver_t::min_covering(const w_graph_t&graph,graph_t&cov)
{
    ...
    {
        assert(match[i]!=i);
        assert(!find_edge(i,match[i],cov));
    ...
    auto remove_match=[&](size_t from,size_t to)
    {
        ...
        {
            auto e=find_edge(from,to,cov); assert(e);
            swap_remove(cov[from],*e);
        }
    };
...
int nonbipartite_solver_t::min_covering(const w_graph_t&graph,graph_t&cov)
{
    ...
    auto save_match=[&](node_t* n)
    {
        assert(!n->is_blossom());
        assert(n->match_node());
        assert(!find_edge(n->index,n->match_edge().second->index,cov));
    ...
    auto remove_match=[&](node_t* from,node_t* to)
    {
        ...
        {
            auto e=find_edge(from->index,to->match_edge().first->index,cov);
            assert(e);
            swap_remove(cov[from->index],*e);
        }
    };
