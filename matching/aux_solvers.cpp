...
int backtracking_solver_t::min_covering(const w_graph_t&graph,graph_t&cover)
{
    ...
    auto set_edge=[&](const std::pair<size_t,size_t>&edge)
    {
        size_t from=edge.first;
        size_t to=graph[from][edge.second].first;
        assert(!find_edge(from,to,cur_cover)&&!find_edge(to,from,cur_cover));
    ...
    auto remove_edge=[&](const std::pair<size_t,size_t>&edge)
    {
        size_t from=edge.first;
        size_t to=graph[from][edge.second].first;
        if(auto from_to=find_edge(from,to,cur_cover);from_to)
        {
            auto to_from=find_edge(to,from,cur_cover);
            assert(to_from);
            swap_remove(cur_cover[from],*from_to);
            swap_remove(cur_cover[to],*to_from);
