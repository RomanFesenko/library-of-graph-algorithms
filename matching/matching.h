...
inline std::optional<std::size_t> find_edge(std::size_t from,std::size_t to,const std::vector<std::vector<std::size_t>>&graph)
{
    auto iter=std::find(graph[from].begin(),graph[from].end(),to);
    if(iter!=graph[from].end()) return iter-graph[from].begin();
    return {};
}

inline std::optional<std::size_t> find_edge(std::size_t from,std::size_t to,const std::vector<std::vector<std::pair<std::size_t,int>>>&graph)
{
    auto iter=std::find_if(graph[from].begin(),graph[from].end(),[&](auto&e){return e.first==to;});
    if(iter!=graph[from].end()) return iter-graph[from].begin();
    return {};
}
