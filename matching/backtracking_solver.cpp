...
const std::size_t* find_edge(const std::vector<std::vector<std::size_t>>&graph,std::size_t from,std::size_t to)
{
    for(const std::size_t& adjacent:graph[from])
    {
        if(adjacent==to) return &adjacent;
    }
    return nullptr;
}

const std::pair<std::size_t,int>*
find_edge(const std::vector<std::vector<std::pair<std::size_t,int>>>&wgraph,std::size_t from,std::size_t to)
{
    for(const std::pair<std::size_t,int>& adjacent:wgraph[from])
    {
        if(adjacent.first==to) return &adjacent;
    }
    return nullptr;
}
...
int backtracking_solver_t::min_covering(const w_graph_t&graph,graph_t&cover)
{
    ...
    auto remove_edge=[&](const std::pair<size_t,size_t>&edge)
    {
        ...
        if(auto from_to=find_edge(cur_cover,from,to);from_to)
        {
            ...
            assert(to_from);
            swap_remove(cur_cover[from],from_to-&cur_cover[from][0]);
            swap_remove(cur_cover[to],to_from-&cur_cover[to][0]);
