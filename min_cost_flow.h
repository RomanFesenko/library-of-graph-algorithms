...
#ifndef _min_cost_flow_
#define _min_cost_flow_
                     ...
                     flow_t req_flow)
{
    ...
    auto search=full_search_t<network_t>{}.set_edge_filter([&](node_t n,edge_t e)
    {
    ...
    {
        sparse_priority_search(wnet,data.source,search,tree,upd);
                    ...
                    flow_t req_flow)
{
    ...
    while(total_flow<req_flow)
    {
        dense_priority_search(wnet,data.source,search,tree,upd);
        if(!tree.in_tree(data.sink)) break;
        cost_t unit_flow_cost=tree.dist(data.sink)+pot[data.sink];
        std::optional<flow_t> added;
        tree.traverse_path(data.sink,[&](node_t node,edge_t edge)
                              ...
                              flow_t req_flow)
{
    ...
    auto wnet=make_weighted_graph(net,residual_cost);
    while(true)
    {
        auto cycle=bellman_ford_negative_cycle(wnet,data.sink,efilter);
