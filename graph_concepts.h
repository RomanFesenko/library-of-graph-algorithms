...
template<class graph_t>
concept w_graph_cpt = requires(graph_t g,typename graph_t::node_handler_t n,
                                         typename graph_t::edge_handler_t e)
{
    graph_cpt<graph_t>;
    typename graph_t::weight_t;

    // graph_t::weight(node_handler_t node,edge_handler_t edge) -
    // return weight of edge in adjacency list of node.
    {g.weight(n,e)}->std::same_as<typename graph_t::weight_t>;
};
