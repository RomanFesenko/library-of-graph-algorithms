...
template<class edge_list_t,class capacity_t>
void edge_list_to_network(const edge_list_t&edges,capacity_t&cap,bool is_direct)
{
    using size_t=std::size_t;
    cap.clear();
    for(auto edge:edges)
    {
        ...
        if(is_direct)
        {
            cap.push_back({from,to,std::get<2>(edge),0});
        }
        else
        {
            cap.push_back({from,to,std::get<2>(edge),std::get<2>(edge)});
        }
    }
}

template<class edge_list_t,class capacity_t>
void graph_to_network(const edge_list_t&edges,capacity_t&cap,bool is_direct)
{
    using size_t=std::size_t;
    cap.clear();
    for(auto edge:edges)
...
void test_breadth_search()
{
    ...
    using graph_t=std::vector<std::vector<std::pair<size_t,size_t>>>;
    {
        ...
        for(size_t test=0;test<suite.size();++test)
        {
            ...
            auto [n,e]=graph_quantities(false,undir_graph,0);
    ...
    }
    {
        ...
        {
            auto [n,e]=graph_quantities(false,gl_graph_suite.undirected[test],0);
            ...
            auto [n2,e2]=graph_quantities(false,gl_graph_suite.tree[test],0);
            ...
            auto [n3,e3]=graph_quantities(false,gl_graph_suite.bipartite[test],0);
...
void test_max_flows()
{
    ...
    {
        graph_test_suite_t suite;
        networks_traits_t<size_t> undir_traits,dir_traits;
        for(size_t test=0;test<suite.size();++test)
        {
            //std::cout<<"F:"<<test<<'\n';
            edge_list_to_network(suite.edge_list(test),undir_traits.upper_cap(),false);
            edge_list_to_network(suite.edge_list(test),dir_traits.upper_cap(),true);
            for(size_t source=0;source<suite.nodes(test);++source)
            {
                for(size_t sink=0;sink<suite.nodes(test);++sink)
                {
                    ...
                    undir_traits.source(source).sink(sink);
                    dir_traits.source(source).sink(sink);

                    assert(min_edges_augmentation(undir_traits)==undir_flow);
                    assert(sparse_max_flow_augmentation(undir_traits)==undir_flow);
                    assert(dense_max_flow_augmentation(undir_traits)==undir_flow);
                    assert(preflow_push_relabel_to_front(undir_traits)==undir_flow);
    ...
    }
    {
        ...
        for(size_t test=0;test<std::min((size_t)10,gl_graph_suite.size);++test)
        {
            ...
            for(size_t source=0;source<std::min((size_t)10,gl_graph_suite.num_nodes);++source)
            {
                ...
                {
                    if(source==sink) continue;
                    traits.source(source).sink(sink);
                    size_t max_flow=min_edges_augmentation(traits);
                    assert(sparse_max_flow_augmentation(traits)==max_flow);
                    assert(dense_max_flow_augmentation(traits)==max_flow);
                    assert(preflow_push_relabel_to_front(traits)==max_flow);
                }
            }
        }
    }
