...
to_adjacency_list(bool is_direct,const std::vector<std::vector<int>>&edges,std::size_t num_nodes)
{
    using size_t=std::size_t;
    std::vector<std::vector<size_t>> list(num_nodes,std::vector<size_t>{});
    for(auto&edge:edges)
    {
        size_t from=edge[0];
        size_t to=edge[1];
        list[from].push_back(to);
        if(!is_direct)  list[to].push_back(from);
    }
    return list;
}

std::vector<std::vector<std::pair<std::size_t,std::size_t>>>
to_weighted_adjacency_list(bool is_direct,const std::vector<std::vector<int>>&edges,std::size_t num_nodes)
{
    using size_t=std::size_t;
    std::vector<std::vector<std::pair<size_t,size_t>>> list(num_nodes,std::vector<std::pair<size_t,size_t>>{});
    for(auto&edge:edges)
    {
        size_t from=edge[0];
        size_t to=edge[1];
        list[from].push_back({to,edge[2]});
        if(!is_direct) list[to].push_back({from,edge[2]});
    }
    return list;
}
...
class Solution_207
{
    ...
    {
        using size_t=std::size_t;
        using adjacency_list_t=std::vector<std::vector<size_t>>;
        std::vector<size_t> nodes(numCourses);
        std::iota(nodes.begin(),nodes.end(),0);
        adjacency_list_t list=to_adjacency_list(true,prerequisites,numCourses);
        auto target=[](auto iter){return *iter;};
        adjacency_list_view_t<adjacency_list_t,decltype(target),size_t> view(list);
        return is_dag(view,nodes,default_index_map_t());
    }
};
...
class Solution_210
{
    ...
    std::vector<int> findOrder(int numCourses, std::vector<std::vector<int>>& prerequisites)
    {
        ...
        if(!total_topological_sort(view,nodes,default_index_map_t{},push)) return {};
        return sorted;
    }
};
...
class Solution_329
{
    ...
    int longestIncreasingPath(const std::vector<std::vector<int>>& matrix)
    {
        ...
        using adjacency_list_t=std::vector<std::vector<size_t>>;
        auto target=[](auto iter){return *iter;};
        using view_t=adjacency_list_view_t<adjacency_list_t,decltype(target),size_t>;
        using edge_t=view_t::edge_handler_t;

        const size_t rows=matrix.size();
        const size_t cols=matrix[0].size();
        auto cell_to_indx=[&](size_t r,size_t c){return r*cols+c+1;};
        adjacency_list_t list(rows*cols+1);
        for(size_t i=0;i<rows;++i)
        {
            for(size_t j=0;j<cols;++j)
            {
                size_t from=cell_to_indx(i,j);
                list[0].push_back(from);
                if(i!=0&&matrix[i-1][j]<matrix[i][j])
                {
                    list[from].push_back(cell_to_indx(i-1,j));
                }
                ...
                {
                    list[from].push_back(cell_to_indx(i,j-1));
                }
        ...
        tree_search_t<size_t,edge_t,size_t,default_index_map_t> tree;
        auto weight_map=[](size_t n,edge_t e){return 1;};
        bool res=dag_priority_search(view_t(list),(size_t)0,full_search_t{},tree,weight_map,minimize_distance_t<size_t,std::greater<>>{});
        assert(res);
        size_t max_dist=0;
        for(size_t node=1;node<list.size();++node)
        {
            assert(tree.opt_dist(node));
            max_dist=std::max(max_dist,*tree.opt_dist(node));
        }
        return max_dist;
    }
};
...
class Solution_743
{
    ...
    int networkDelayTime(const std::vector<std::vector<int>>& times, int n, int k)
    {
        ...
        adjacency_list_t list=to_weighted_adjacency_list(true,times,n+1);
        tree_search_t<size_t,edge_t,size_t,default_index_map_t> tree;
        auto weight_map=[](size_t n,edge_t e){return e->second;};
        sparse_dijkstra(view_t(list),k,full_search_t{},tree,weight_map);
        size_t time=0;
        for(int node=1;node<=n;++node)
        {
            if(!tree.opt_dist(node)) return -1;
            time=std::max(time,*tree.opt_dist(node));
        }
        return time;
    }
};
...
class Solution_882
{
    ...
    int reachableNodes(const std::vector<std::vector<int>>& edges, int maxMoves, int n)
    {
        ...
        using view_t=adjacency_list_view_t<adjacency_list_t,decltype(target),size_t>;
        using edge_t=view_t::edge_handler_t;
        adjacency_list_t list(n);
        for(auto&edge:edges)
        {
            size_t n1=edge[0];
            size_t n2=edge[1];
            size_t subdiv=edge[2];
            list[n1].push_back({n2,subdiv+1});
            list[n2].push_back({n1,subdiv+1});
        }
        tree_search_t<size_t,edge_t,size_t,default_index_map_t> tree;
        size_t reachable=0;
        auto searcher=full_search_t{}.set_edge_filter([&](size_t n,edge_t e)
        {
            return tree.dist(n)+e->second<=(size_t)maxMoves;
        })
        ...
        auto weight_map=[](size_t n,edge_t e){return e->second;};
        dense_dijkstra(view_t(list),0,searcher,tree,weight_map);
        for(auto&edge:edges)
        {
            size_t n1=edge[0];
            size_t n2=edge[1];
...
class Solution_1632
{
    ...
    std::vector<std::vector<int>> matrixRankTransform(const std::vector<std::vector<int>>& matrix)
    {
        ...
        for(size_t i=0;i<rows;++i)
        {
            for(size_t j=0;j<cols;++j)
            {
                ...
                for(size_t k=0;k<cols;++k)
                {
                    ...
                    {
                        list[from].push_back({to,1});
                    }
                    else if(matrix[i][j]==matrix[i][k])
                    {
                        list[from].push_back({to,0});
                        list[to].push_back({from,0});
                    }
                    else
                    {
                        list[to].push_back({from,1});
                    }
                }
        ...
        tree_search_t<size_t,edge_t,size_t,default_index_map_t> tree;
        auto weight_map=[](size_t n,edge_t e){return e->second;};
        bool res=bellman_ford_priority_search(view_t(list),0,
                                              tree,
                                              weight_map,
                                              always_bool_t<true>{},
                                              minimize_distance_t<size_t,std::greater<>>{});
        assert(res);
...
class Solution_1334
{
    ...
    int findTheCity(int n,const std::vector<std::vector<int>>& edges, int distanceThreshold)
    {
        ...
        adjacency_list_t list=to_weighted_adjacency_list(false,edges,n);
        tree_search_t<size_t,edge_t,size_t,default_index_map_t> tree;
        std::vector<size_t> reachable(n,0);
        size_t source;
        size_t city=0;
        auto search=full_search_t{}.set_edge_filter([&](size_t node,edge_t edge)
        {
            return tree.dist(node)+edge->second<=(size_t)distanceThreshold;
        })
        .set_node_preprocess([&](size_t )
        {
            reachable[source]++;
        ...
        {
            dense_dijkstra(view_t(list),source,search,tree,weight_map);
...
class Solution_1368
{
    ...
    int minCost(const std::vector<std::vector<int>>& grid)
    {
        ...
        adjacency_list_t list(rows*cols);
        for(size_t i=0;i<rows;++i)
        {
            for(size_t j=0;j<cols;++j)
            {
                ...
                if(i!=0)
                {
                    list[from].push_back({cell_to_indx(i-1,j),(grid[i-1][j]==upper? 0:1)});
                }
                if(i!=rows-1)
                {
                    list[from].push_back({cell_to_indx(i+1,j),(grid[i][j]==lower? 0:1)});
                }
                if(j!=0)
                {
                    list[from].push_back({cell_to_indx(i,j-1),(grid[i][j]==left? 0:1)});
                }
                if(j!=cols-1)
                {
                    list[from].push_back({cell_to_indx(i,j+1),(grid[i][j]==right? 0:1)});
                }
        ...
        tree_search_t<size_t,edge_t,size_t,default_index_map_t> tree;
        auto weight_map=[](size_t n,edge_t e){return e->second;};
        sparse_dijkstra(view_t(list),0,full_search_t{},tree,weight_map);
...
class Solution_1557
{
    ...
    std::vector<int> findSmallestSetOfVertices(int n,const std::vector<std::vector<int>>& edges)
    {
        ...
        adjacency_list_t list=to_adjacency_list(true,edges,n);
        view_t view(list);
        std::vector<char> processed(n,0);
        auto search=full_search_t{}.set_node_preprocess([&](size_t node)
        {
            processed[node]=1;
            return true;
        })
        .set_edge_filter([&,view](size_t node,edge_t edge)
        {
            return !processed[view.target(node,edge)];
        });
        std::vector<size_t> top_sorted;
        std::vector<size_t> nodes(n);
        std::iota(nodes.begin(),nodes.end(),0);
        auto push=[&](size_t node ){top_sorted.push_back(node);};
        bool dag=total_topological_sort(view,nodes,imap,push);
        assert(dag);
        ...
        for(size_t source:top_sorted)
        {
            if(!processed[source])
            {
                ...
                breadth_search(true,view,source,search,imap);
            }
        }
...
class Solution_1584
{
    ...
    int minCostConnectPoints(const std::vector<std::vector<int>>& points)
    {
        ...
        std::vector<size_t> adjacency(points.size());
        std::iota(adjacency.begin(),adjacency.end(),0);
        adjacency_list_t list(points.size(),adjacency);
        view_t view(list);

        auto weight_map=[&,view](size_t n,edge_t e)
        {
            size_t target=view.target(n,e);
            return std::abs(points[n][0]-points[target][0])+
                   std::abs(points[n][1]-points[target][1]);
        };
        tree_search_t<size_t,edge_t,size_t,default_index_map_t> tree;
        return dense_prim(view,0,tree,weight_map).first;
    }
};
