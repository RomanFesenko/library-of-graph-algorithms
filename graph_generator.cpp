...
std::pair<std::size_t,std::size_t> graph_generator_t::edge_range(std::size_t nodes,graph_type_t type)
{
    assert(nodes>=2);
    std::pair<std::size_t,std::size_t> res;
    switch(type)
    {
        case undirected_graph_id:
        res= {nodes-1,nodes*(nodes-1)/2};
        break;

        case directed_graph_id:
...
bool graph_generator_t::request_t::is_valid()const
{
    if(num_nodes<2) return false;
    auto r=edge_range(num_nodes,type);
    return num_edges>=r.first&&num_edges<=r.second;
}


bool graph_generator_t::make_graph(graph_t&graph,const request_t&req)
{
    if(!req.is_valid()) return false;
    graph.clear();
    graph.resize(req.num_nodes,{});
    switch(req.type)
    {
        case undirected_graph_id:
        m_make_undirected_graph(graph,req.num_nodes,req.num_edges);
        break;

        case directed_graph_id:
        m_make_directed_graph(graph,req.num_nodes,req.num_edges);
        break;

        case tree_id:
        m_make_undirected_tree(graph,req.num_nodes);
        break;

        case bipartite_id:
        m_make_bipartite(graph,req.num_nodes,req.num_edges);
        break;

        case dag_id:
        m_make_dag(graph,req.num_nodes,req.num_edges);
        break;
